...
def _duck_frames() -> list[str]:
    """A rubber duck — round yellow body, orange beak, cute."""
    frame1 = _build_sprite([
        "ttttyytttttt",
        "tttyyyyyyttt",
        "ttykyyyykytt",
    ...
    # happy duck — eyes closed, beak open
    frame3 = _build_sprite([
        "ttttyytttttt",
        "tttyyyyyyttt",
        "ttyyyyyyyyytt",
...
def _fox_frames() -> list[str]:
    """A fox — orange with white muzzle, pointy ears, bushy tail."""
    frame1 = _build_sprite([
        "tootttttoott",
        "toootttooott",
        "tooooooooot",
...
def _axolotl_frames() -> list[str]:
    """An axolotl — pink with feathery external gills, wide head, stumpy legs."""
    frame1 = _build_sprite([
        "rpttttttprtt",
        "rpptttttpprtt",
        "trppppppprttt",
    ...
    # axolotl — happy (eyes squinted, big smile)
    frame3 = _build_sprite([
        "rpttttttprtt",
        "rpptttttpprtt",
        "trppppppprttt",
...
def _dragon_frames() -> list[str]:
    """A dragon — green with TWO eyes, horns, wing stubs, fire breath."""
    frame1 = _build_sprite([
        "tyGttttGytt",
        "tGGGttGGGtt",
        "ttGGGGGGttt",
    ...
    # dragon — resting, eyes half closed
    frame4 = _build_sprite([
        "tyGttttGytt",
        "tGGGttGGGtt",
        "ttGGGGGGttt",
...
def _unicorn_frames() -> list[str]:
    """A unicorn — white, golden horn, TWO eyes, pink mane, four legs."""
    frame1 = _build_sprite([
        "ttttyytttttt",
        "ttttywtttttt",
        "tpptwwtttttt",
...
def _ghost_frames() -> list[str]:
    ...
    # Frame 3: ghost happy — eyes squinted, smile
    frame3 = _build_sprite([
        "ttttwwwwtttt",
        "tttwwwwwwttt",
        "ttwwwwwwwwtt",
...
def _cosmic_whale_frames() -> list[str]:
    """A cosmic whale — blue, horizontal whale shape, tail flukes, stars."""
    frame1 = _build_sprite([
        "ytttttBBtttytttt",
        "tttttBBBtttttttt",
        "ttBBBBBBBBBttttt",
...
def _bee_frames() -> list[str]:
    """A busy little bee — yellow/black stripes, white wings, antennae."""
    frame1 = _build_sprite([
        "tttktttkttt",
        "tttkttkttttt",
        "tttyyyyttttt",
...
def _raccoon_frames() -> list[str]:
    """A raccoon — gray with black eye mask, striped tail attached to body."""
    frame1 = _build_sprite([
        "teettteetttt",
        "teeetteeett",
        "tteeeeeeettt",
    ...
    # raccoon — happy (eyes squinted, mask visible)
    frame3 = _build_sprite([
        "teettteetttt",
        "teeetteeett",
        "tteeeeeeettt",
...
def _parrot_frames() -> list[str]:
    """A colorful parrot — green body, red head, yellow beak, perched."""
    frame1 = _build_sprite([
        "ttttrggttttt",
        "tttrrgggtttt",
        "tttrggggttt",
...
def _wolf_frames() -> list[str]:
    """A wolf — gray, tall pointed ears, long snout, body with legs."""
    frame1 = _build_sprite([
        "teettttteetttttt",
        "teeettteeetttttt",
        "tteeeeeeeetttttt",
    ...
    # wolf — happy (eyes squinted, teeth showing)
    frame3 = _build_sprite([
        "teettttteetttttt",
        "teeettteeetttttt",
        "tteeeeeeeetttttt",
...
def _robot_frames() -> list[str]:
    """A robot — boxy metallic, antenna, screen on chest, angular limbs."""
    frame1 = _build_sprite([
        "tttttetttttt",
        "tttttetttttt",
        "teeeeeeeeettt",
...
def _void_cat_frames() -> list[str]:
    """A cat from the void — dark with pointed ears, glowing eyes, tail."""
    frame1 = _build_sprite([
        "tEEtttttEEttt",
        "tEEEtttEEEttt",
        "ttEEEEEEEtttt",
    ...
    # void_cat — happy (eyes glow brighter, purring)
    frame3 = _build_sprite([
        "tEEtttttEEttt",
        "tEEEtttEEEttt",
        "ttEEEEEEEtttt",
...
def _dolphin_frames() -> list[str]:
    """A dolphin — side view, curved body, dorsal fin, pointed snout."""
    frame1 = _build_sprite([
        "tttttttbbttttttttt",
        "tttttttbbbttttttttt",
        "tttbbbbbbbbbbtttttt",
    ...
    # dolphin — sleepy (half-closed eye)
    frame4 = _build_sprite([
        "tttttttbbttttttttt",
        "tttttttbbbttttttttt",
        "tttbbbbbbbbbbtttttt",
...
def _orca_frames() -> list[str]:
    """An orca — side view, tall dorsal fin, black top/white belly, eye patch."""
    frame1 = _build_sprite([
        "tttttttkkttttttttt",
        "ttttttkkkttttttttt",
        "tttttkkkktttttttt",
...
def _chonk_frames() -> list[str]:
    """The legendary chonk — orange cat with ears, maximum round body, tiny legs."""
    frame1 = _build_sprite([
        "ttootttttootttt",
        "ttoootttoootttt",
        "ttoooooooootttt",
    ...
    # chonk sitting — maximum loaf mode
    frame3 = _build_sprite([
        "ttootttttootttt",
        "ttoootttoootttt",
        "ttoooooooootttt",
...
def _panda_frames() -> list[str]:
    ...
    # panda — happy (eyes squinted, keep black patches)
    frame3 = _build_sprite([
        "ttkktttkktttt",
        "ttwwwwwwwtttt",
        "twwwwwwwwttt",
...
def _gorby_frames() -> list[str]:
    # Round pink blob — Kirby-like, with red shoes
    frame1 = _build_sprite([
        "ttttttttttttt",
        "ttttppppttttt",
        "tttpppppptttt",
...
def _corgi_frames() -> list[str]:
    ...
    # corgi — happy (eyes squinted, nose visible)
    frame3 = _build_sprite([
        "ttNNttNNttt",
        "tNNNNNNNNtt",
        "tNNNNNNNNtt",
...
def _claude_frames() -> list[str]:
    """Claw'd — the little orange 8-bit mascot from Claude Code."""
    frame1 = _build_sprite([
        "toooooooooottt",
        "toooooooooottt",
        "ookooookoottt",
...
def _mimic_frames() -> list[str]:
    """A Dark Souls mimic chest — closed and then OPEN with tongue and legs."""
    ...
    # Frame 2: OPEN with teeth, tongue, and proper legs
    frame2 = _build_sprite([
        "tYYYYYYYYYt",
        "YYYYYYYYYYt",
        "YwtwtwtwYYt",
...
def _moth_frames() -> list[str]:
    """A moth — fuzzy body, feathery antennae, large patterned wings."""
    frame1 = _build_sprite([
        "tNttttttNtttt",
        "ttNtttNttttt",
        "tttNNNtttttt",
...
def _snail_frames() -> list[str]:
    """A snail — eye stalks, spiral shell on back, green slimy body."""
    frame1 = _build_sprite([
        "tkttktttttttt",
        "tkttktttttttt",
        "tettettttttt",
...
def _rat_frames() -> list[str]:
    """A sneaky little rat — big ears, no legs, tail off to the right."""
    frame1 = _build_sprite([
        "teeetteeetttt",
        "teeeeteeettt",
        "tteeeeeeetttt",
...
def _kobold_frames() -> list[str]:
    """A kobold — small reptilian with horns, snout, scaly, holds a spear."""
    frame1 = _build_sprite([
        "tYotttttYotttt",
        "ttoooooootttt",
        "tooooooooott",
    ...
    # kobold — happy (eyes squinted, toothy grin)
    frame3 = _build_sprite([
        "tYotttttYotttt",
        "ttoooooootttt",
        "tooooooooott",
...
def _joe_camel_frames() -> list[str]:
    """Joe Camel — big head with cool sunglasses, long face, smirk."""
    frame1 = _build_sprite([
        "tttNNNNNttttt",
        "ttNNNNNNNtttt",
        "tNNNNNNNNNttt",
...
def _comrade_frames() -> list[str]:
    """Hammer and sickle with googly eyes on a red banner."""
    frame1 = _build_sprite([
        "rrrrrrrrrrrrtt",
        "rrryyrrrrrrrtt",
        "rryyyyrrrrrrt",
...
def _coopa_frames() -> list[str]:
    """A turtle troopa — green shell, yellow head poking out, stubby legs."""
    frame1 = _build_sprite([
        "ttyyyyttttttt",
        "ttykykyttttt",
        "ttyyyyygttttt",
